...
- A public security and trust flow that addresses credential-handling concerns before demo submission.
...
- Contact form email delivery via SMTP (`/contact` and `/api/contact` in Node server; Netlify function available).
- Homepage hero micro-form submits directly to `/contact` with source-aware payload handling.
- Dedicated `Security` page (`/security`) with FAQ-style trust messaging and status badges (`Operational`, `In progress`, `Planned`).
- Inline credential-security reassurance at the Step 1 trust-anxiety trigger in `How It Works`.
- Reusable content modules for conversion/trust copy in `src/content/socialProof.js` and `src/content/securityTrust.js`.
...
- Pages:
  - Public: `Home`, `About`, `Contact`, `Security`.
...
6. Public demo intake can come from:
   - Hero micro-form (`source: "hero"`, work email + trust name), or
   - Contact page form (`source: "contact"`, full demo request form).
7. Contact endpoint sends SMTP notification to configured recipients.
...
| POST | `/contact` or `/api/contact` | No | Send contact/demo request email (hero + contact payload variants) |

#### Contact Payload Shapes (Current)

Hero micro-form payload:

```json
{
  "workEmail": "jane@nhs.uk",
  "trustOrHospital": "Leeds Teaching Hospitals",
  "source": "hero"
}
```
...
│   ├── content/                  # Shared marketing/security copy modules
│   │   ├── socialProof.js
│   │   └── securityTrust.js
│   └── pages/Security.jsx        # Dedicated public credential security page
├── scripts/
...
| --- | --- | --- |
| Public conversion UX | Operational (hero micro-form + social proof flow) | Operational (iterate with analytics and copy tests) |
| Credential trust messaging | Operational (`/security` page + inline callouts + status badges) | Operational (tighten with verified compliance evidence) |
